    ...
    <q-form @submit="saveData">
      <q-card-section class="row q-pa-sm">
        <q-item class="full-width">
          <q-item-section>
            <q-item-label class="text-h6 text-weight-bolder" lines="1">{{ submitForm.id ? $t('update_product_type') : $t('add_new_product_type') }}</q-item-label>
          </q-item-section>
          <q-item-section side>
      ...
      <q-card-section class="q-pa-sm ">
        <q-list class="row">
          ...
          <q-item class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
            <q-item-section>
              <q-input dark color="white" dense v-model="submitForm.name" :label="$t('product_type_name')"
                :rules="[val => val && val.length > 0 || 'Please enter product type name']" />
            </q-item-section>
          </q-item>
    ...
    saveData: async function () {
      ...
      try {
        ...
        if (this.submitForm.id) {
          res = await jq.post(ref.apiUrl('api/v1/admin/ajax/update_product_type_data'), this.submitForm);
        } else {
          res = await jq.post(ref.apiUrl('api/v1/admin/ajax/store_product_type_data'), this.submitForm);
        }
        this.notify(res.msg)
