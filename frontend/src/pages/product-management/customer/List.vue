<template>
  <q-page class="q-pa-sm">
    <q-breadcrumbs class="text-grey q-mb-sm q-mt-sm" active-color="green">
      ...
      <q-breadcrumbs-el :label="$t('customer_list')" />
    </q-breadcrumbs>
    <q-card class="no-shadow" bordered>
      <q-card-section>
        <div class="row">
          <div class="text-h6 col-10 text-grey-8">{{ $t('customer_list') }}</div>
    ...
    <q-dialog v-model="showAddNewDialog" position="right">
      <create-customer :title="editItem.id ? $t('update_customer') : $t('add_new_customer')" :companies="companies"
...
import createCustomer from "./AddOrUpdate.vue";
...
export default {
  ...
  computed: {
    ...
    columns: function () {
      return [
        ...
        { name: "address", label: this.$t('address'), field: "address" },
        { name: "website", label: this.$t('website'), field: "website" },
  ...
  methods: {
    ...
    getListData: async function () {
      ...
      try {
        this.loading = true
        let res = await jq.get(ref.apiUrl('api/v1/admin/ajax/get_customer_list'));
    ...
    deleteDataConfirmed: async function (item) {
      ...
      try {
        let res = await jq.post(ref.apiUrl('api/v1/admin/ajax/delete_customer_data'), item);
        this.notify(res.msg)
        this.getListData()
