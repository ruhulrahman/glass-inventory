<template>
  <q-page class="q-pa-sm">
    ...
    <q-breadcrumbs class="text-grey q-mb-sm q-mt-sm" active-color="green">
      ...
      <q-breadcrumbs-el :label="$t('weekend_list')" />
    </q-breadcrumbs>

    <q-card class="no-shadow" bordered>
      <q-card-section>
        <div class="row">
          <div class="text-h6 col-10 text-grey-8">{{ $t('weekend_list') }}</div>
    ...
    <q-dialog v-model="showAddNewDialog" position="right">
      <add-or-update ref="weekend_modal" :editItem="editItem" @reloadListData="getListData"
...
const metaData = { title: 'Weekend List' }
...
export default ({
  ...
  computed: {
    ...
    columns: function () {
      return [
        { name: "day_name", required: true, label: this.$t('weekend_name'), align: "left", field: (row) => row.name, format: (val) => `${val}`, sortable: true, align: "center" },
  ...
  methods: {
    ...
    getListData: async function () {
      ...
      try {
        this.loading = true
        let res = await jq.get(ref.apiUrl('api/v1/admin/ajax/get_weekend_list'));
    ...
    deleteDataConfirmed: async function (item) {
      ...
      try {
        let res = await jq.post(ref.apiUrl('api/v1/admin/ajax/delete_weekend_data'), item);
        this.notify(res.msg)
        this.getListData()
