<template>
  <q-page class="q-pa-sm">
    <q-breadcrumbs class="text-grey q-mb-sm q-mt-sm" active-color="green">
      ...
      <q-breadcrumbs-el :label="$t('permission_list')" />
    </q-breadcrumbs>
    <q-card class="no-shadow" bordered>
      <q-card-section>
        <div class="row">
          <div class="text-h6 col-10 text-grey-8">{{ $t('permission_list') }}</div>
          ...
          <template v-slot:body="props">
            <q-tr :props="props">
              ...
              <q-td key="parent" :props="props">
                <span v-if="props.row.parent">{{ props.row.parent }}</span>
              </q-td>
...
export default {
  ...
  computed: {
    ...
    columns: function () {
      return [
        ...
        { name: "code", label: this.$t('code'), field: "code", align: "left" },
        { name: "parent", label: this.$t('parent'), field: "parent", align: "left" },
  ...
  methods: {
    ...
    getParentPermissionListData: async function () {
      let ref = this;
      let jq = ref.jq();
      try {
        this.loading = true
        let res = await jq.get(ref.apiUrl('api/v1/admin/ajax/get_permission_parent_list'));
    ...
    deleteDataConfirmed: async function (item) {
      ...
      try {
        let res = await jq.post(ref.apiUrl('api/v1/admin/ajax/delete_permission_data'), item);
        this.notify(res.msg)
        this.getListData()
