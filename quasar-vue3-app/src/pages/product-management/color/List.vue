<template>
  <q-page class="q-pa-sm">
    ...
    <q-breadcrumbs class="text-grey q-mb-sm q-mt-sm" active-color="green">
      ...
      <q-breadcrumbs-el label="Color" />
    </q-breadcrumbs>

    <q-card class="no-shadow" bordered>
      <q-card-section>
        <div class="row">
          <div class="text-h6 col-10 text-grey-8">Color List</div>
    ...
    <q-dialog v-model="showAddNewDialog" position="right">
      <add-or-update :title="'Create Color'" :editItem="editItem"
...
const metaData = { title: 'Color List' }
...
export default ({
  ...
  methods: {
    ...
    getListData: async function () {
      ...
      try {
        this.loading = true
        let res = await jq.get(ref.apiUrl('api/v1/admin/ajax/get_product_color_list'));
    ...
    deleteDataConfirmed: async function (item) {
      ...
      try {
        let res = await jq.post(ref.apiUrl('api/v1/admin/ajax/delete_product_color_data'), item);
        this.notify(res.msg)
        this.getListData()
