...
class AjaxController extends Controller
{
    public function get(Request $req, $name)
    {
        ...
        if ($name == 'get_auth_user') {
            ...
            $query = model('ProductInvoice')::with('customer', 'payment_status', 'details')
                ->where('company_id', $user->company_id);

            if (!has_permission($user, 'show_vat_check_option')) {
                $query->where('vat_user_can_see', 1);
            }
            ...
            $productInvoice->vat_user_can_see = $productInvoice->vat_user_can_see ? TRUE : FALSE;
            ...
            $dashboardData->total_user = model('User')::where(['active' => 1, 'company_id' => $user->company_id])->count();
    ...
    public function post(Request $req, $name)
    {
        ...
        if ($name == 'store_department_data') {
            ...
            $employee->update([
                ...
                'active' => $req->active == 'false' ? 0 : 1,
                'current_salary' => $req->current_salary == 'null' ? NULL : $req->current_salary,
                'updated_at' => Carbon::now()
            ]);
            ...
            try {
                ...
                $productInvoice = model('ProductInvoice')::create([
                    ...
                    'vat_user_can_see' => $req->vat_user_can_see == 'true' ? 1 : 0,
                    'creator_id' => $user->id,
                ]);
            ...
            try {
                ...
                if ($productInvoice) {
                    ...
                    $productInvoice->vat_user_can_see = $req->vat_user_can_see == 'true' ? 1 : 0;
                    $productInvoice->save();
